import type { CacheMatcher, ImageCachingPlugin, ImageCachingPluginAPI } from '@/types'
import { Vault, normalizePath } from 'obsidian'
import { Logger, URI, URL } from 'obsidian-fnc'
...
export class ImageCachingAPI implements ImageCachingPluginAPI {
  #log = new Logger('ImageCachingAPI')
  ...
  async resolve(notepath: string, remote: string): Promise<string | undefined> {
    if (!URL.isUrl(remote)) {
    ...
    if (!ext) throw new ImageError('remote-no-ext')
