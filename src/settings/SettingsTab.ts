...
export class SettingsTab extends PluginSettingTab {
  ...
  #displayGeneralSettings(): void {
    ...
    charsSetting.setDesc('If you are having problems with special characters on image paths, disable this setting.')
    ...
    urlignoreSetting.setDesc('Overrides standard rules and ignores the file.')
  ...
  #displayConfigsHeader(): void {
    ...
    headerEl.addText((input) => {
      ...
      input.onChange((value) => {
        ...
        // handle validation
        const problems = checkPattern(this.#settings.cache_configs, value)
        if (problems.length > 0) {
          problems.forEach((p) => headerDescUl.createEl('li').appendText(p))
          return
        }
    ...
    headerEl.addButton((button) => {
      ...
      button.onClick(() => {
        ...
        const src = this.#settings.cache_configs.find((i) => i.pattern === ref)
        if (!match || !src) {
